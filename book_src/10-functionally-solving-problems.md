...
Let's go over the expression `10 4 3 + 2 * -`{.fixed} together! First we
...
We could also use a triple of `(Int, Int, Int)`{.fixed} to represent a
...
We left fold over `roadSystem`{.fixed} (remember, it's a list of
