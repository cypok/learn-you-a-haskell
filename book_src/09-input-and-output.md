...
Because of that, `main`{.fixed} always has a type signature of
...
What about `main`{.fixed}? First, we get a line from the terminal by
...
The second line is the input. We input `hello sir`{.fixed} and then
...
A common pattern with `sequence`{.fixed} is when we map functions like
...
`forM`{.label .function} (located in `Control.Monad`{.fixed}) is like
...
We could have actually done that without `forM`{.fixed}, only with
...
`getContents`{.fixed} is really useful when we're piping the output from
...
So what we're essentially doing with that use of `forever`{.fixed} is
...
Just like we have `hGetContents`{.fixed} that works like
...
The `System.Environment`{.fixed} module has two cool I/O actions. One is
...
Great! All that's left now is to implement `add`{.fixed}, `view`{.fixed}
...
We opened up the file based on `fileName`{.fixed} and opened a temporary
...
To use our `random`{.fixed} function, we have to get our hands on one of
...
Why doesn't `randoms`{.fixed} return a new generator as well as a list?
...
![jack of diamonds](lyah/jackofdiamonds.png)
...
We make our own function that takes two `FilePath`{.fixed}s (remember,
...
.function} function from `System.Directory`{.fixed}.
...
We did `fileExists <- doesFileExist fileName`{.fixed} because
`doesFileExist`{.fixed} has a type of
`doesFileExist :: FilePath -> IO Bool`{.fixed}, which means that it
returns an I/O action that has as its result a boolean value which tells
us if the file exists. We can't just use `doesFileExist`{.fixed} in an
...
the `catch`{.label .function} function from `System.IO.Error`{.fixed}.
Its type is `catch :: IO a -> (IOError -> IO a) -> IO a`{.fixed}. It
...
so-called handler. If the first I/O action passed to `catch`{.fixed}
throws an I/O exception, that exception gets passed to the handler,
which then decides what to do. So the final result is an I/O action that
will either act the same as the first parameter or it will do what the
handler tells it if the first I/O action throws an exception.

![non sequitor](lyah/puppy.png)

If you're familiar with *try-catch* blocks in languages like Java or
Python, the `catch`{.fixed} function is similar to them. The first
...
The handler takes a value of type `IOError`{.fixed}, which is a value
...
`IOError`{.fixed} by pattern matching against them, just like we can't
pattern match against values of type `IO something`{.fixed}. We can use
a bunch of useful predicates to find out stuff about values of type
`IOError`{.fixed} as we'll learn in a second.

So let's put our new friend `catch`{.fixed} to use!
...
infix function makes it more readable. So
`` toTry `catch` handler ``{.fixed} is the same as
`catch toTry handler`{.fixed}, which fits well with its type.
`toTry`{.fixed} is the I/O action that we try to carry out and
`handler`{.fixed} is the function that takes an `IOError`{.fixed} and
returns an action to be carried out in case of an exception.
...
In the handler, we didn't check to see what kind of `IOError`{.fixed} we
got. We just say `"Whoops, had some trouble!"`{.fixed} for any kind of
...
from `System.IO.Error`{.fixed} — `isDoesNotExistError`{.label .function}
and `ioError`{.label .function}. `isDoesNotExistError`{.fixed} is a
predicate over `IOError`{.fixed}s, which means that it's a function that
takes an `IOError`{.fixed} and returns a `True`{.fixed} or
`False`{.fixed}, meaning it has a type of
`isDoesNotExistError :: IOError -> Bool`{.fixed}. We use it on the
exception that gets passed to our handler to see if it's an error caused
by a file not existing. We use
[guard](syntax-in-functions.html#guards-guards) syntax here, but we
could have also used an *if else*. If it's not caused by a file not
existing, we re-throw the exception that was passed by the handler with
the `ioError`{.fixed} function. It has a type of
`ioError :: IOException -> IO a`{.fixed}, so it takes an
`IOError`{.fixed} and produces an I/O action that will throw it. The I/O
action has a type of `IO a`{.fixed}, because it never actually yields a
result, so it can act as `IO anything`{.fixed}.

So the exception thrown in the `toTry`{.fixed} I/O action that we glued
together with a *do* block isn't caused by a file existing,
`` toTry `catch` handler ``{.fixed} will catch that and then re-throw
it. Pretty cool, huh?

There are several predicates that act on `IOError`{.fixed} and if a
guard doesn't evaluate to `True`{.fixed}, evaluation falls through to
the next guard. The predicates that act on `IOError`{.fixed} are:
...
Most of these are pretty self-explanatory. `isUserError`{.fixed}
evaluates to `True`{.fixed} when we use the function `userError`{.label
...
`ioError $ userError "remote computer unplugged!"`{.fixed}, although
It's prefered you use types like `Either`{.fixed} and `Maybe`{.fixed} to
...
Where `notifyCops`{.fixed} and `freeSomeSpace`{.fixed} are some I/O
actions that you define. Be sure to re-throw exceptions if they don't
match any of your criteria, otherwise you're causing your program to
fail silently in some cases where it shouldn't.

`System.IO.Error`{.fixed} also exports functions that enable us to ask
our exceptions for some attributes, like what the handle of the file
that caused the error is, or what the filename is. These start with
`ioe`{.fixed} and you can see a [full list of
...
error. We can't print the `fileName`{.fixed} that we got from
`getArgs`{.fixed}, because only the `IOError`{.fixed} is passed to the
handler and the handler doesn't know about anything else. A function
depends only on the parameters it was called with. That's why we can use
the `ioeGetFileName`{.label .function} function, which has a type of
`ioeGetFileName :: IOError -> Maybe FilePath`{.fixed}. It takes an
`IOError`{.fixed} as a parameter and maybe returns a `FilePath`{.fixed}
(which is just a type synonym for `String`{.fixed}, remember, so it's
kind of the same thing). Basically, what it does is it extracts the file
path from the `IOError`{.fixed}, if it can. Let's modify our program to
...
In the guard where `isDoesNotExistError`{.fixed} is `True`{.fixed}, we
used a *case* expression to call `ioeGetFileName`{.fixed} with
`e`{.fixed} and then pattern match against the `Maybe`{.fixed} value
that it returned. Using *case* expressions is commonly used when you
want to pattern match against something without bringing in a new
function.

You don't have to use one handler to `catch`{.fixed} exceptions in your
whole I/O part. You can just cover certain parts of your I/O code with
`catch`{.fixed} or you can cover several of them with `catch`{.fixed}
and use different handlers for them, like so:
...
Here, `toTry`{.fixed} uses `handler1`{.fixed} as the handler and
`thenTryThis`{.fixed} uses `handler2`{.fixed}. `launchRockets`{.fixed}
isn't a parameter to `catch`{.fixed}, so whichever exceptions it might
throw will likely crash our program, unless `launchRockets`{.fixed} uses
`catch`{.fixed} internally to handle its own exceptions. Of course
`toTry`{.fixed}, `thenTryThis`{.fixed} and `launchRockets`{.fixed} are
I/O actions that have been glued together using *do* syntax and
hypothetically defined somewhere else. This is kind of similar to
*try-catch* blocks of other languages, where you can surround your whole
...
like `IO (Either a b)`{.fixed}, meaning that they're normal I/O actions
but the result that they yield when performed is of type
