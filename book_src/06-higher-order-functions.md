...
If we call it with `99`{.fixed}, it returns a `GT`{.fixed}. Simple
...
Pretty simple stuff. If `p x`{.fixed} evaluates to `True`{.fixed}, the
...
like the one we'd get by writing
`[(0*),(1*),(2*),(3*),(4*),(5*)..`{.fixed}.
...
How about `sum (filter (> 10) (map (*2) [2..10]))`{.fixed}? Well,
